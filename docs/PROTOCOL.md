...
| LCD bus speed | **400 kHz** (`NV_I2C_SPEED` enum value `NVAPI_I2C_SPEED_400KHZ`, not literal kHz). GCC's `AorusLcdService` pins the panel bus to 400 kHz; the NVAPI `NVAPI_I2C_SPEED_DEFAULT` (0) leaves the controller's current, unspecified speed, at which the panel intermittently rejects otherwise-valid writes (status -1) and the content freezes. See `NvApiI2cSpeed`, `NvApiI2cBus.BuildInfo`, `NvApiPanelLocator`. |
